import { IdCheckRequestDto, SignInRequestDto } from "./dto/request/auth";
...
import { GetCommentResponseDto, GetDiaryResponseDto, GetEmpathyResponseDto } from "./dto/response/diary";
import { GetSignInUserResponseDto } from "./dto/user";
...
export const idCheckRequest = async (requestBody: IdCheckRequestDto) => {
    const resopnseBody = await axios.post(ID_CHECK_URL, requestBody)
...
export const signUpRequest = async (requestBody: SignUpRequestDto) => {
    const resopnseBody = await axios.post(SIGN_UP_URL, requestBody)
    .then(responseSuccessHandler)
    .catch(responseErrorHandler);
    return resopnseBody;
};
