...
function TableItem({ concentrationTest }: TableItemProps) { 
    ...
    // variable: 오류 차이 문자열 //
    const errorGapText = errorGap === null ? '' : errorGap > 0 ? `+${errorGap}` : `${scoreGap}`;
    ...
    return (
        <div className='tr'>
            ...
            <div className='th measurement-score'>{measurementScore}</div>
            <div className='th score-gap'>{scoreGapText}</div>
            <div className='th error-count'>{errorCount}</div>
            <div className='th error-gap'>{errorGapText}</div>
        </div>
    )
}
